...
const double tick_nano_sec = CLOCKS_PER_SEC / 1000000.0;
...
void test_liste_gabow() {
    ...
    for (int i = 500; i < 30000; i = i + 10) {
        ...
        for(int j = 0; j < 10; j++) {
            ...
            valeurs.emplace_back(duration);
            free(gabow);
        }
        long double moyenne = 0.0;
        for(auto i = valeurs.begin(); i != valeurs.end(); i++){
            moyenne += (*i / tick_nano_sec);
        }
        std::cout << i << " " << moyenne / valeurs.size() << std::endl;
        free(graphe);
    }
}
...
int main(int argc, char *argv[]) {
    if (argc > 1) {
        ...
        printf("\t-liste : graphe de liste d\'adjacence\n");
        printf("\t-adjacence : graphe de matrice d\'adjacence\n");
        printf("\n");
        printf("\tAlgorithme:\n");
